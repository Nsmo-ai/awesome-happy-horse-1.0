...
- 一边是公开产品与对外展示层
- 一边是公开 benchmark 与社媒讨论层

这会导致一个常见问题：讨论热度远远大于公开发布完整度。
...
1. `2026-04-08`
   - Artificial Analysis 在公开讨论里把 HappyHorse-1.0 变成了高可见 benchmark 话题。

2. `2026-04-08` 到 `2026-04-09`
   - X 上高传播账号放大 benchmark 叙事与比较框架。
...
- **[Likely]** 公开产品层面的叙事完整度明显强于公开技术发布完整度。
...
| 信号地图 | [Public Signal Surfaces](./docs/public-signal-surfaces.md) | 看 benchmark、社媒、产品层与站点说法之间的关系 |
...
3. `X / Reddit 讨论层`
   - 看 narrative、比较、rumor 是如何被放大的

4. `站点说法层`
   - 看哪些网址只是传播噪声，哪些适合作为用户入口
...
因为它同时具备 benchmark 可见度和社媒传播扩散，两条传播路径叠加后，讨论速度非常快。详见 [Timeline](./docs/timeline.md) 与 [Benchmarks](./docs/benchmarks.md)。

### Happy Horse 开源了吗？

按照当前这条证据链，没有看到被验证的权重或推理代码公开发布。详见 [Claims Ledger](./docs/claims-ledger.md)。
